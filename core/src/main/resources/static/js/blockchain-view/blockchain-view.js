var BlockchainView = (function () {
    ...
    var blockHashFun = function (b) {
        return b.blockHash
    };
    var hashCompareFun = function (hash) {
        return function (b) {
    ...
    function prepareAndAddBlocks(newBlocks) {
        ...
        blockNumbers.sort(function (a, b) {
            return a - b;
        });
        ...
        if (renderColumns.length == 0) {
            ...
            var putOverParent = renderColumns.some(function (column) {
                return column.some(function (b) {
                    if (b.blockHash == block.parentHash) {
                        var blockOverParent = column.some(function (bb) {
                            return bb.index == block.index
                        });
                        if (!blockOverParent) {
                            column.push(block);
                            sortByIndex(column);
            ...
            if (!putOverParent) {
                ...
                var putInFreeSpace = renderColumns.some(function (column) {
                    var blockAtPlace = column.some(function (bb) {
                        return bb.index == block.index
                    });
                    if (!blockAtPlace) {
                        column.push(block);
    ...
    function renderState(svgContainer, width, height) {
        ...
        blockGroupSelection
            ...
            .attr("transform", function (d) {
                return "translate(" + d.x + "," + (d.y - BLOCK_HEIGHT - GAP) + ")"
            })
        ...
        var numberingSelection = numberingContainer
            .selectAll('text')
            .data(blockNumberObjects, function (d) {
                return d.text;
            });
        ...
        numberingSelection
            ...
            .append('text')
            .attr('x', function (d) {
                return d.x + NUMBERING_WIDTH / 2;
            })
            .attr('y', function (d) {
                return d.y + 20 - BLOCK_HEIGHT - GAP;
            })
            .text(function (d) {
                return d.text;
            })
