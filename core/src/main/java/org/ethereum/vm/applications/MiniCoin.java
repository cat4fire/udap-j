...
public class MiniCoin {
    ...
    key 10^10 + 600 issue fixed or elastic
    key 10^10 + 601 total mint
    key 10^10 + 602 transfer-able

    key 10^10 + 700 pointer to next mint coin

    key 10^10+1000 TitleString length
    ...
    public void miniCoinCreate() {
        ...
        api.setString(new DataWord(account), new DataWord(0), content);
        String title = (String) params[1];
        api.setString(new DataWord(account), new DataWord(10000000000l + 1000), title);
        ...
        dataWords.add(new DataWord("miniCoinCreate"));
        LogInfo logInfo = new LogInfo(account, dataWords, account);
        List<DataWord> dataWords2 = new ArrayList<>();
        dataWords2.add(new DataWord("miniCoinCreateCoin"));
        LogInfo logInfo2 = new LogInfo(account, dataWords2, BigInteger.valueOf(i).toByteArray());
    ...
    public void miniCoinMint() {
        ...
        for (int i = 0; i < amount; i++) {
            api.program.storageSave(new DataWord(account), new DataWord(pointer + i), new DataWord(receiver));
            List<DataWord> l = new ArrayList<>();
            l.add(new DataWord("miniCoinMint"));
            logInfos.add(new LogInfo(account, l, new DataWord(pointer + i).getData()));
        }
        api.program.getResult().addLogInfos(logInfos);
        return;
    }
